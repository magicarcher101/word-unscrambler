...
export default async function WordsStartingWithPage({ params }) {
  const { letter } = await params;
    ...
    <main className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <nav className="text-sm text-gray-500 mb-6">
          ...
          <span>Words Starting With {letter.toUpperCase()}</span>
        </nav>
        ...
        <p className="text-gray-600 mb-6">
          Found <strong>{allWords.length.toLocaleString()}</strong> words starting with <strong>{letter.toUpperCase()}</strong>. Valid for Scrabble and Words with Friends.
        </p>

        <div className="flex flex-wrap gap-2 mb-8">
          {LETTERS.map(l => (
            <Link key={l} href={`/words-starting-with/${l}`}
        ...
        {lengths.map(len => (
          <div key={len} id={`length-${len}`} className="mb-8">
            <h2 className="text-lg font-semibold text-gray-800 mb-3 border-b border-gray-200 pb-2">
              {len}-letter words starting with {letter.toUpperCase()} ({grouped[len].length})
            </h2>
