...
function loadWords(letter) {
  try {
    ...
    return words.filter(w => w.length === 5 && w.startsWith(letter.toLowerCase()));
  } catch { return []; }
}

export default async function FiveLetterWordsStartingWithPage({ params }) {
  const { letter } = await params;
    ...
    <main className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <nav className="text-sm text-gray-500 mb-6">
          ...
          <Link href="/5-letter-words-starting-with/a" className="hover:text-blue-600">5 Letter Words</Link>
          <span className="mx-2">›</span>
        ...
        <p className="text-gray-600 mb-6">
          Found <strong>{words.length.toLocaleString()}</strong> five-letter words starting with <strong>{letter.toUpperCase()}</strong>. Perfect for Wordle, Scrabble, and Words with Friends.
        </p>

        {/* Letter navigation */}
        <div className="flex flex-wrap gap-2 mb-8">
          {LETTERS.map(l => (
            <Link key={l} href={`/5-letter-words-starting-with/${l}`}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-8">
          <p className="text-sm text-blue-800">
            <strong>🟩 Wordle tip:</strong> If you know your word starts with <strong>{letter.toUpperCase()}</strong>, 
            try one of the high-scoring options below to eliminate as many letters as possible.
          </p>
        </div>

        {/* Word grid */}
        {words.length === 0 ? (
          <div className="text-center py-12 text-gray-400">No 5-letter words found starting with {letter.toUpperCase()}.</div>
        ) : (
        ...
        <div className="mt-10 pt-6 border-t border-gray-200">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href={`/5-letter-words-ending-with/${letter}`}
