    ...
    <header className="bg-blue-600 text-white py-4 shadow-md">
      ...
      {menuOpen && (
        <div className="md:hidden bg-blue-700 px-4 py-3 flex flex-col gap-3 text-sm font-medium">
          ...
          <Link href="/words-starting-with/a" onClick={() => setMenuOpen(false)} className="hover:text-blue-200 transition-colors">
            📖 Words Starting With
          </Link>
          <Link href="/words-ending-with/a" onClick={() => setMenuOpen(false)} className="hover:text-blue-200 transition-colors">
            📖 Words Ending With
          </Link>
        </div>
      )}
